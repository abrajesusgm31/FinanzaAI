...
import { Plus, Search, Filter, ArrowUpRight, ArrowDownLeft, ArrowLeftRight, Tag } from 'lucide-react';
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-3 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors"
          >
            <Tag className="w-3.5 h-3.5 text-indigo-600" /> Nueva Categoría
          </button>
          <button
            onClick={() => setIsTxModalOpen(true)}
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-colors shadow-sm"
          >
              ...
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                    No hay transacciones registradas en este workspace.
                  </td>
                </tr>
              )}
            </tbody>
